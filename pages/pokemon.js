...
const Pokemon = ({ pokemonProp }) => {
    ...
    const dataOptions = [
        ...
        { key: 'Peso:', value: pokemon.weight, suffix: 'Kg.' },
        { key: 'Altura:', value: pokemon.height, suffix: 'M.' },
        { key: 'Especie:', value: pokemon.specie, suffix: '' }
    ]

    return (
        <Layout title={ `${ nameToTitleCase(pokemon.name) } - Alex Cantón` }>
            <div className={`flex flex-row flex-nowrap border-${pokemon.types[0].type.name} rounded-xl p-4`}>
                <div className="w-1/2">
